...
<task type="auto">
  <name>Task 1: Prepare verification data</name>
  <files></files>
  <action>
Before browser verification, ensure production has suitable test data.
...
**Step 3: Document IDs for verification**
```bash
# Get documents to verify document links work
curl -s "$BACKEND_URL/api/documents/" | jq '.documents[] | {id, filename, status}'
```

Create a verification notes file with the IDs to test.
  </action>
  <verify>
```bash
# Verify API returns data
BACKEND_URL="https://loan-backend-prod-fjz2snvxjq-uc.a.run.app"
curl -s "$BACKEND_URL/api/borrowers/" | jq '.total'
# Should be > 0
```
  </verify>
  <done>
- Borrower IDs recorded for testing
- Source reference data structure verified
- Document IDs available for link testing
  </done>
</task>

<task type="checkpoint:human-verify" gate="blocking">
  <what-built>
Plan 21-01 deployed API and frontend changes that expose char_start/char_end fields and add visual highlighting in the UI. This checkpoint verifies all four TEST requirements through browser-based inspection.
  </what-built>
  <how-to-verify>
**Production URLs:**
...
3. **VERIFY:**
   - [ ] If char_start/char_end are present in data:
     - Visual indicator (badge, highlight, or icon) shows "exact match" or similar
     - Text may be highlighted with yellow background
   - [ ] If char_start/char_end are null:
     - Standard snippet display (no highlighting)
...
**Expected result:** API returns char_start/char_end fields; UI differentiates exact vs non-exact extractions
...
1. Navigate to borrower detail page with income records
   (If no borrower has income, note this in results)
...
- UI shows differentiation: YES/NO
- Observations: [what you saw]
...
<task type="auto">
  ...
  <action>
Based on human verification results from Task 2, create a comprehensive verification report.
...
[User observations from checkpoint]
