...
Output: Evidence that frontend loads correctly and uploads succeed
</objective>
...
<task type="checkpoint:human-verify" gate="blocking">
  <name>Task 3: Verify document upload flow (TEST-02)</name>
  <what-built>
  ...
  <how-to-verify>
    ...
    8. Verify:
       - [ ] Document details show filename
       - [ ] Status shows "completed" (may take a few seconds)
       - [ ] extraction_method shows "docling"
  </how-to-verify>
  <resume-signal>
    Type "TEST-02 PASS" if upload flow works correctly.
    Type "TEST-02 FAIL: [description]" if there are issues.

    Include document ID from successful upload for next plan.
  </resume-signal>
</task>
...
| TEST-02 | Upload document | Success message, document in list |
