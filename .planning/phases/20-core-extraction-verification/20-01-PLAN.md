...
<task type="auto">
  <name>Task 1: Configure database connection</name>
  <files>Secret Manager: database-url</files>
  <action>
    Configure the database-url secret to point to a valid loan database.

    **Option A - Create new database (if Cloud SQL instance has access):**
    ```bash
    ...
    **Option B - Use existing database with migrations:**
    If user prefers existing database, update connection string and run migrations.
...
<task type="checkpoint:human-action" gate="blocking">
  <name>Task 2: Provide Gemini API key</name>
  <action>
...
<task type="auto">
  <name>Task 3: Update Gemini API key secret</name>
...
Include:
- Database configuration method used
