...
files_modified: []
autonomous: false
state_file: ".planning/phases/20-core-extraction-verification/20-01-STATE.json"
...
<state_protocol>
This plan uses a state file for passing information between checkpoint and auto tasks.

**State file location:** `.planning/phases/20-core-extraction-verification/20-01-STATE.json`
...
```json
{
  "task1_decision": {
    ...
    "database_name": "loan_extraction|[user-specified]",
    "timestamp": "ISO8601"
  },
  "task2_result": {
    "secret_version": "N",
...
**Protocol:**
1. Checkpoint tasks WRITE their outputs/decisions to state file
2. Auto tasks READ required inputs from state file before execution
3. State file enables autonomous plan resumption and audit trail
</state_protocol>
...
<task type="checkpoint:decision" gate="blocking">
  ...
  <state_output>
    After user responds, write to state file:
    ```json
    {
      "task1_decision": {
        "option": "[user choice: option-a or option-b]",
        "database_name": "[loan_extraction or user-specified name]",
...
<task type="auto">
  ...
  <state_input>
    Before execution, read from state file:
    ```bash
    cat .planning/phases/20-core-extraction-verification/20-01-STATE.json | jq '.task1_decision'
    ```
    Extract: option, database_name
  </state_input>
  <action>
    **Step 0: Read decision from state file**
    ```bash
    DECISION=$(cat .planning/phases/20-core-extraction-verification/20-01-STATE.json | jq -r '.task1_decision.option')
    DB_NAME=$(cat .planning/phases/20-core-extraction-verification/20-01-STATE.json | jq -r '.task1_decision.database_name')
    ```
    ...
    **If option-b (use existing database):**
    Update connection string with user-specified database name from state file.
    ...
    **Write result to state file:**
    ```bash
    # Update state file with Task 2 results
    jq '.task2_result = {"secret_version": "N", "api_status": "200", "timestamp": "'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' \
      .planning/phases/20-core-extraction-verification/20-01-STATE.json > tmp.json && mv tmp.json .planning/phases/20-core-extraction-verification/20-01-STATE.json
    ```
  </action>
  <verify>
    ```bash
  ...
  <done>
    ...
    - State file updated with task2_result
  </done>
</task>
...
<task type="checkpoint:human-action" gate="blocking">
  ...
  <state_output>
    After user provides key, write to state file:
    ```json
    {
      "task3_input": {
        "gemini_api_key": "[USER PROVIDED KEY]",
    ...
    ```
    Note: Key is stored temporarily for Task 4 execution, then removed from state file after secret update.
  </state_output>
...
<task type="auto">
  ...
  <state_input>
    Before execution, read API key from state file:
    ```bash
    GEMINI_KEY=$(cat .planning/phases/20-core-extraction-verification/20-01-STATE.json | jq -r '.task3_input.gemini_api_key')
    ```
  </state_input>
  <action>
    **Step 0: Read API key from state file**
    ```bash
    GEMINI_KEY=$(cat .planning/phases/20-core-extraction-verification/20-01-STATE.json | jq -r '.task3_input.gemini_api_key')
    ```
    ...
    **Step 2: Clean up sensitive data from state file**
    ```bash
    # Remove API key from state file (security)
    jq 'del(.task3_input.gemini_api_key) | .task4_result = {"secret_version": "N", "timestamp": "'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' \
      .planning/phases/20-core-extraction-verification/20-01-STATE.json > tmp.json && mv tmp.json .planning/phases/20-core-extraction-verification/20-01-STATE.json
    ```
  ...
  <verify>
    ...
    # Verify state file has result and no sensitive data
    cat .planning/phases/20-core-extraction-verification/20-01-STATE.json | jq '.task4_result'
    cat .planning/phases/20-core-extraction-verification/20-01-STATE.json | jq '.task3_input.gemini_api_key'
  ...
  <done>
    ...
    - State file updated with task4_result
    - API key removed from state file (security cleanup)
...
3. State file complete:
   ```bash
   cat .planning/phases/20-core-extraction-verification/20-01-STATE.json | jq .
   # Should show task1_decision, task2_result, task4_result
   # Should NOT contain gemini_api_key value
   ```
</verification>
...
- [ ] State file contains decision audit trail
- [ ] No sensitive data left in state file
</success_criteria>
...
- State file final contents (minus sensitive data)
</output>
