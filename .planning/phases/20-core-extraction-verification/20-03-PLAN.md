...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
    **Step 1: Trigger OCR processing**
    ...
    5. Note the document ID returned
    ...
    **Step 3: Verify OCR was invoked (check backend logs)**
    ```bash
    # Check backend logs for OCR service invocation
    gcloud logging read "resource.type=cloud_run_revision AND resource.labels.service_name=loan-backend-prod AND textPayload:\"ocr\" OR textPayload:\"lightonocr\"" --limit=20 --format="table(timestamp,textPayload)" --project=memorygraph-prod

    # Expected: Log entries showing:
    # - "Calling OCR service" or similar
    # - "OCR request sent to https://lightonocr-gpu..."
    # - "OCR processing complete" or similar
    ```

    **Step 4: Verify GPU service received the request**
    ```bash
    # Check GPU service logs for incoming request
    gcloud logging read "resource.type=cloud_run_revision AND resource.labels.service_name=lightonocr-gpu" --limit=10 --format="table(timestamp,textPayload)" --project=memorygraph-prod
    ...
    **Step 5: Verify final document state**
    ```bash
    curl -s "https://loan-backend-prod-fjz2snvxjq-uc.a.run.app/api/documents/DOC_ID" | jq '{ocr_processed, status, extraction_method}'
    # Expected: {"ocr_processed": true, "status": "completed", ...}
    ```
  ...
  <resume-signal>
    ...
    Include:
    - Processing time observed (e.g., "cold start: 90s, processing: 15s")
    - Backend log excerpt showing OCR service call
    - GPU service log excerpt showing request received (if available)
    - Document ocr_processed value (should be true)
  </resume-signal>
</task>
...
| TEST-05 | GPU OCR | ocr_processed=true AND logs confirm OCR service was called |

All three must pass for Phase 20 completion.

**Note on TEST-05:** Verification requires BOTH:
1. Document shows ocr_processed=true (final state)
2. Logs confirm the connection worked (backend called GPU service, GPU service processed request)
</verification>
...
- [ ] TEST-05 logs: Backend logs show OCR service invocation
- [ ] TEST-05 logs: GPU service logs show request was received and processed
...
- Backend log excerpts showing OCR service call
- GPU service log excerpts showing request processing
