...
<objective>
Archive Terraform configuration to /archive/terraform/ with complete documentation.

Purpose: CloudBuild will replace Terraform for deployments. Archiving configuration ensures we can reference original setup and prevents accidental terraform destroy. Resources remain in GCP - only the management tool changes. State remains in GCS remote backend (not copied locally).
Output: Complete Terraform archive with configs and migration documentation.
</objective>
...
Do NOT copy .terraform directory or any .tfstate files (state is in GCS remote backend and remains there - not archived locally).
  </action>
...
State is NOT destroyed or copied - resources remain managed by GCP.
The .tfstate files are NOT archived locally because:
1. State contains sensitive outputs (connection strings, IPs)
