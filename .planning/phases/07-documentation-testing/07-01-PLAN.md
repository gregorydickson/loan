...
must_haves:
  ...
  key_links:
    ...
    - from: "docs/SYSTEM_DESIGN.md"
      ...
      via: "Chunking and pipeline parameters verified against code"
      pattern: "4000.*token|200.*overlap"
---
...
**Verification step:** Cross-check documented patterns against actual code:
- Verify chunking values (4000 tokens, 200 overlap) match `extractor.py`
- Verify model names match `llm_client.py` constants
- Verify deduplication order matches implementation
  </action>
  <verify>
File contains:
...
```bash
# Verify documented chunking matches code
grep -r "4000\|4096" backend/src/extraction/ && echo "Chunking documented correctly"
# Verify model names documented match code
grep -r "gemini.*flash\|gemini.*pro" backend/src/extraction/ --ignore-case
```
  </verify>
  <done>
