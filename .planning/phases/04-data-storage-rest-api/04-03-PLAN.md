...
@.planning/phases/04-data-storage-rest-api/04-02-SUMMARY.md
...
<task type="auto">
  <name>Task 1: Create borrower API endpoints and wire dependencies</name>
  <files>backend/src/api/borrowers.py, backend/src/api/dependencies.py, backend/src/main.py</files>
  <action>
Create backend/src/api/borrowers.py with all borrower endpoints AND wire the dependencies in the same task.
...
**Update dependencies.py (same task since it's trivial):**
...
**Update main.py (same task since it's trivial):**
...
2. borrowers_router
  ...
  <verify>
Run: `cd /Users/gregorydickson/stackpoint/loan/backend && python -c "from src.main import app; paths = [r.path for r in app.routes if 'borrower' in r.path]; print('Borrower routes:', paths)"`
Should show all borrower routes registered.
  </verify>
  <done>borrowers.py has 4 endpoints, BorrowerRepoDep wired in dependencies.py, borrowers_router registered in main.py</done>
</task>

<task type="auto">
  <name>Task 2: Create unit tests for borrower endpoints</name>
