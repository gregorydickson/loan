...
must_haves:
  truths:
    ...
    - "Year validation checks income years are reasonable (1950-current+1)"
  ...
  artifacts:
    - path: "backend/src/extraction/validation.py"
      provides: "FieldValidator with validate_ssn, validate_phone, validate_zip, validate_year"
...
Purpose: Ensure data quality - validate formats (SSN, phone, zip, year), calculate confidence scores to flag uncertain extractions for review
...
1. `ValidationError` dataclass:
   - `field: str` - Field name (ssn, phone, zip_code, year)
   - `value: str` - The invalid value
   - `error_type: str` - Error category (FORMAT, LENGTH, INVALID, NANP, RANGE)
...
3. `FieldValidator` class with compiled regex patterns:
   ...
   - `MIN_YEAR = 1950` - Reasonable minimum year for income records
...
- `validate_year(self, year: int | None) -> ValidationResult`:
  - None returns valid
  - `current_year = datetime.now(timezone.utc).year`
  - If year < MIN_YEAR -> RANGE error "Year {year} is before {MIN_YEAR}"
  - If year > current_year + MAX_YEAR_OFFSET -> RANGE error "Year {year} is in the future"
  - Otherwise valid

NOTE: VALID-04 "Validator checks date formats" is addressed by validate_year() because income records use `year: int` not date strings. The year validation ensures income years are reasonable.
...
- `test_year_valid_current` - Current year is valid
- `test_year_valid_past` - 2020 is valid
- `test_year_invalid_too_old` - 1900 returns RANGE error
- `test_year_invalid_future` - Current year + 5 returns RANGE error
- `test_year_allows_next_year` - Current year + 1 is valid (projected income)
...
`cd backend && python -m pytest tests/extraction/test_validation.py -v` passes
  ...
  <done>
FieldValidator validates SSN, phone, ZIP, year formats with detailed error tracking
  </done>
</task>
...
<success_criteria>
- FieldValidator validates SSN (XXX-XX-XXXX), phone (via phonenumbers), zip (XXXXX[-XXXX]), year (1950-current+1)
