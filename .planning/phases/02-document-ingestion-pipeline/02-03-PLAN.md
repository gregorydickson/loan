...
must_haves:
  truths:
    ...
    - "Failed GCS upload updates document status to FAILED with error message"
    - "Processing errors do not crash the application"
    - "Upload returns immediately (processing is async via Cloud Tasks in Phase 3)"
  artifacts:
...
Purpose: Enable file uploads through the REST API with duplicate detection, GCS storage, and database tracking. Uploads return immediately with PENDING status - actual Docling processing is asynchronous via Cloud Tasks (implemented in Phase 3).

**IMPORTANT ARCHITECTURAL NOTE (Blocker 2 Resolution):**
Docling processing takes 5-60 seconds per document based on research findings. Synchronous processing in the upload endpoint would block the API for unacceptable durations. Therefore:
- Upload creates DB record (PENDING) + stores file in GCS + returns immediately
- Actual Docling processing happens via Cloud Tasks queue handler (Phase 3 scope)
- This plan does NOT call DoclingProcessor.process() in the upload flow

Output:
- DocumentService orchestrating upload flow (no Docling processing - that's Phase 3)
- POST /api/documents endpoint accepting file uploads
- File hash computation for duplicate detection
- Error handling for duplicates and GCS failures
- Unit tests for service layer
- Integration tests for API endpoint including error scenarios
</objective>
...
<task type="auto">
  <name>Task 1: DocumentService with upload orchestration (no Docling processing)</name>
  <files>
    backend/src/ingestion/document_service.py
    backend/tests/unit/test_document_service.py
  </files>
...
**CRITICAL (Blocker 2 Resolution):**
This service handles upload orchestration ONLY. It does NOT call DoclingProcessor.process().
Docling processing happens asynchronously via Cloud Tasks, which will be implemented in Phase 3.
...
5. Upload to GCS
6. Return document record (still PENDING)

Phase 3 will add:
- Queue processing task to Cloud Tasks after GCS upload
- Cloud Tasks handler that calls DoclingProcessor.process()
- Status updates to PROCESSING -> COMPLETED/FAILED
...
- Test verifies upload does NOT call DoclingProcessor (test_upload_does_not_call_docling)
- Test verifies upload returns PENDING status (test_upload_success_returns_pending)
  </verify>
  <done>
DocumentService orchestrates upload flow with hash computation, duplicate detection, GCS storage, and database tracking. Document returns with PENDING status - Docling processing is async via Cloud Tasks (Phase 3). Errors are handled gracefully with proper status updates. All unit tests pass.
  </done>
</task>
...
- `ruff check backend/src/api/` passes
  ...
  <done>
API module created with POST /api/documents (upload), GET /api/documents/{id} (detail), GET /api/documents (list). Upload returns immediately with status=pending - processing is async. Dependencies provide GCS client and DocumentService injection. OpenAPI docs available at /docs.
  </done>
</task>

<task type="auto">
  <name>Task 3: Integration tests for document upload API including error handling</name>
  <files>
    backend/tests/integration/__init__.py
    backend/tests/integration/conftest.py
    backend/tests/integration/test_documents_api.py
  ...
  <action>
Create integration tests for the document API, including error handling scenarios (INGEST-14 coverage):
...
class TestDocumentUploadErrorHandling:
    ...
    async def test_gcs_failure_returns_500_not_crash(self, client_with_failing_gcs: AsyncClient):
        ...
        assert "upload" in response.json()["detail"].lower() or "storage" in response.json()["detail"].lower()
    ...
    async def test_missing_filename_handled(self, client: AsyncClient):
        ...
        # Should handle gracefully (use "unknown" as filename or reject)
...
- Tests verify INGEST-14 (graceful error handling): GCS failure, unsupported types, duplicates, not found all return proper HTTP codes without crashing
- Tests verify async model: upload returns 'pending' status, not 'completed'
  </verify>
  <done>
Integration tests cover all document API endpoints with proper database and GCS mocking. Tests verify upload flow returns PENDING status (async processing model), duplicate detection, document retrieval, and listing with pagination. Error handling tests verify INGEST-14 (graceful errors without crashes).
  </done>
</task>
...
8. Test get document - returns document with status='pending' (not 'completed')
</verification>

<success_criteria>
1. POST /api/documents accepts multipart file upload
2. Upload returns immediately with status='pending' (processing is async)
3. SHA-256 hash computed before database insert
4. Duplicate files (same hash) rejected with 409 Conflict
5. Successful upload creates document record with status PENDING
6. Successful upload stores file in GCS (or mock) with gs:// URI
7. GCS failures update document status to FAILED with error message (INGEST-14)
8. GET /api/documents/{id} returns document details
9. GET /api/documents returns paginated list
10. Failed operations return appropriate HTTP status codes without crashing (INGEST-14)
